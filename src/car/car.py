...
class Car(pg.sprite.Sprite):
    def __init__(self):
        ...
        self.image = pg.transform.scale(
            pg.image.load(path_car), (50, 50)).convert_alpha()
    ...
    def build_radars(self, collisions, angle):
        self.radars.clear()
        # Draw 7 radars for an angle of 30 degree each
        for degree in range(-90, 120, 30):
            length, size_pad = 0, 5
            # Use this pad to check collisions of the radars
            pad = pg.Rect(
                self.rect.center[0], self.rect.center[1], size_pad, size_pad)
            ...
            while not pad.collidelist(collisions) >= 0 and length < 180:
                ...
                x = int(
                    self.rect.center[0] + math.cos(math.radians(360 - (angle - degree))) * length)
                y = int(
                    self.rect.center[1] + math.sin(math.radians(360 - (angle - degree))) * length)
                length += 1
